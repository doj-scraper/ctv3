    ...
    <div className="relative overflow-hidden px-4 pb-16 pt-24 sm:px-6 lg:px-12">
      ...
      <div className="relative mx-auto grid min-h-[calc(100dvh-8rem)] max-w-6xl items-center gap-8 lg:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
        <section className="animate-rise-in space-y-6">
          <div className="text-micro text-ct-accent">Account Access</div>
          <div className="space-y-4">
            <h1 className="heading-display text-4xl text-ct-text sm:text-5xl lg:text-6xl">
              SIGN IN TO THE <span className="text-ct-accent">CELLTECH</span> RAIL
            </h1>
            <p className="max-w-xl text-sm leading-7 text-ct-text-secondary sm:text-base">
              Return to the live catalog, cart, and order ledger with the same operational shell used across
              the storefront.
            </p>
          </div>

          <div className="grid gap-3 sm:grid-cols-2">
            <RailCard title="Live catalog" copy="Search inventory, filters, and fitment from one entry point." />
            <RailCard title="Protected orders" copy="Access your account ledger and checkout history after sign-in." />
          </div>

          <div className="flex flex-wrap items-center gap-3">
            <Link href="/sign-up" className="text-sm uppercase tracking-[0.16em] text-ct-accent transition-colors hover:text-ct-text">
              Create Account
            </Link>
